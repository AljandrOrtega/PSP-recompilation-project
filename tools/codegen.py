...
def main(argv):
    ...
    # Retail EBOOTs can have their section-name table stripped (e.g. The 3rd Birthday), so
    # .sceStub.text is not findable by name. Fall back to the import table's stub addresses.
    def is_stub(a):
        if stub is not None:
            return stub["addr"] <= a < stub["addr"] + stub["size"]
        return a in impmap
